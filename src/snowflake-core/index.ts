...
export class SnowflakeCore extends pulumi.ComponentResource {
  ...
  constructor(
    name: string,
    args: SnowflakeCoreArgs,
    opts?: pulumi.ComponentResourceOptions,
  ) {
    super("pkg:index:SnowflakeCore", name, opts);

    this.database = new snowflake.Database(
      "database",
      {
    ...
    this.warehouses = args.warehouses.map(
      (warehouse) => {
    ...
    this.user = new snowflake.User(
      "user",
      {
        ...
        defaultWarehouse: this.warehouses.find((wh) => wh.isDefault)!!
          .snowflakeObject.name,
    ...
    const deploymentRoleGrant = new snowflake.RoleGrants(
      "role->deployment",
      {
        roleName: this.role.name,
        roles: ["DEPLOYMENT"],
    ...
    new snowflake.SchemaGrant(
      "role->public-schema-ownership",
      {
