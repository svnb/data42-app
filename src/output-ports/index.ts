...
export class OutputPort extends pulumi.ComponentResource {
    ...
    constructor(name: string, args: OutputPortArgs, opts?: pulumi.ComponentResourceOptions) {
        super("pkg:index:OutputPort", name, opts)

        this.output = args.name

        this.schema = new snowflake.Schema(args.name, {
        ...
        this.role = new snowflake.Role(`${args.name}`, {
            name: `${args.app}_${args.name}`.toUpperCase()
        }, { parent: this })

        new snowflake.RoleGrants(`${args.name}->role`, {
            roleName: this.role.name,
            roles: [args.snowflake.role.name]
        }, { parent: this })

        new snowflake.GrantPrivilegesToRole(`${args.name}-database-usage`, {
            privileges: ['USAGE'],
            roleName: this.role.name,
            onAccountObject: {
                objectType: 'DATABASE',
        ...
        new snowflake.GrantPrivilegesToRole(`${args.name}-schema-usage`, {
            privileges: ['USAGE'],
            roleName: this.role.name,
            onSchema: {
                schemaName: pulumi.all([args.snowflake.database.name, this.schema.name]).apply(([database, schema]) => `${database}.${schema}`)
            }
        }, { parent: this })
        ...
        outputObjects.forEach(object => {
            ...
            new snowflake.GrantPrivilegesToRole(`${args.name}-${object}->select`, {
                privileges: ["SELECT"],
                roleName: this.role.name,
                onSchemaObject
            }, { parent: this })
            ...
            new snowflake.GrantPrivilegesToRole(`role->${args.name}-${object}->create`, {
            ...
            new snowflake.GrantPrivilegesToRole(`role->${args.name}-${object}->ownership`, {
                privileges: ["OWNERSHIP"],
                roleName: this.role.name,
                onSchemaObject
            }, { parent: this, dependsOn: args.snowflake }) // Wait until Role Hierarchy (role -> deployment from snowflake module) is created
        })
